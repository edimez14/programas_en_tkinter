...
text1 = Label(window, text='hour', bg='#77fdca',
              fg='black', font=('Arial', 12, 'bold'))
text1.grid(row=1, column=0, padx=5, pady=5)
text2 = Label(window, text='minute', bg='#77fdca',
              fg='black', font=('Arial', 12, 'bold'))
text2.grid(row=1, column=1, padx=5, pady=5)
text3 = Label(window, text='seconds', bg='#77fdca',
              fg='black', font=('Arial', 12, 'bold'))
...
combobox1 = ttk.Combobox(window, values=list_hour, style="TCombobox",
                         justify='center', width='12', font='Ariel')
combobox1.grid(row=2, column=0, padx=15, pady=5)
combobox1.current(0)
combobox2 = ttk.Combobox(window, values=list_minute,
                         style="TCombobox", justify='center', width='12', font='Ariel')
combobox2.grid(row=2, column=1, padx=15, pady=5)
combobox2.current(0)
combobox3 = ttk.Combobox(window, values=list_seconds,
                         style="TCombobox", justify='center', width='12', font='Ariel')
...
amount = ttk.Combobox(window, values=(1, 2, 3, 4, 5),
                      justify='center', width='8', font=('Arial'))
    ...
    if int(minutes) == int(x_minutes):
        if int(hour) == int(x_hour):
            if int(seconds) == int(x_seconds):
                mixer.music.load(
                    "y2mate.com - The Big Bang Theory IntroLyrics.mp3")
